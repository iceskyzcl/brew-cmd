...
> 普通关键字包括: 缓冲区列表、导入文件以及标签文件的单词

> 重复补全按键可连续补全

> 补全技巧:`<C-n><C-p>`不关闭补全列表继续输入, 精简列表


## 寄存器
- `:reg`: 查看寄存器列表
- `"{reg}{cmd}`: 调用{reg}寄存器进行剪切/复制/粘贴
  - `"0{cmd}`: 复制专用寄存器, 命令`y`复制时会同时往`"0`寄存器复制

默认复制/剪切等使用无名寄存器 unnamed
