...
正常模式下，操作=操作符+<a href='#motion'>选择范围</a>，连续调用两次操作符则作用于当前行。
- `d移动`: 剪切至移动位置
...
## 命令行模式
- `:[range]d`: delete删除指定范围的行
  - `:{start},{end}d`: 删除第{start}行至第{end}行，缺省为当前行，$末行，%所有行
- `:[range]y`: yank复制指定范围的行
- `:[range]j`: join连接指定范围内的行
- `:[range]m{address}`: move把指定范围内的行移动到 {address} 指定的行之下
- `:[range]t{address}`: copy把指定范围内的行复制到 {address} 指定的行之下
- `:[range]s/{pattern}/{string}/[flags]`: substitute把指定范围内出现{pattern}的地方替换为{string}
  - `:s/old/new/`: 替换当前行的第一个匹配
  - `:s/old/new/g`: 替换当前行的所有匹配
  - `:%s/old/new/`: 替换所有行的第一个匹配
  - `:%s/old/new/gc`: 替换所有行的所有匹配，逐个确认
  - `:1,9s/^/    /`: 第1行至第9行每行前加四个空格
- `[range]g/{pattern}/[cmd]`: global对指定范围内匹配{pattern}的所有行执行命令{cmd}
- `[range]v/{pattern}/[cmd]`: vglobal对指定范围内**不匹配**{pattern}的所有行执行命令{cmd}

> 可视模式下选择了范围，则[range]默认为选区范围`'<,'>`
